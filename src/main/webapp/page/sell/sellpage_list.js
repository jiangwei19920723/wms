layui.config({
    ...
    function tab(res){
    	table.render({
            ...
            ,cols: [[ //表头
                ...
                ,{field: 'arrears', title: '未付金额（元）',width:140,event: 'setArrears',style:'cursor: pointer;'}
    ...
    $(".btn").click(function(){
    	...
    	if (value ==0) {
			...
			var res = synAjaxJson("GET", '/v0.1/sell/selectByArrears', null, "查询失败！");
			tab(res);
		}
    });
    /**
    ...
    table.on('tool(doEven)', function(obj){
        ...
        if(obj.event === 'setArrears'){
            layer.prompt({
              formType: 2
              ,title: '修改 销售单号 为 ['+ data.id +'] 的未付金额（元）'
              ,value: data.arrears
              ...
              data.arrears =value;
              var res = undefined;
              res = synAjax("PUT", '/v0.1/sell/updateSellArrears', data, '修改失败!');
              if (res ==undefined ) {
				return false;
              }else {
            	  layer.msg("修改成功！");
