layui.config({
    ...
    function getDataCallBack() {
    	var res = synAjax("GET", '/v0.1/inport/list', null, '提交失败!');
    	tab(res);
        return false;
    }
    /**
    ...
    $(".btn").click(function(){
    	...
    	if (value ==0) {
			...
			var res = synAjaxJson("GET", '/v0.1/inport/select', 'startTime='+start_time+'&endTime='+end_time, "查询失败！");
			tab(res);
		}else if (value == 1) {
			var code = $("#code").val();
			var res = synAjaxJson("GET", '/v0.1/inport/select', 'goodsCode='+code, "查询失败！");
			tab(res);
		}else if (value == 2) {
			var name = $("#name").val();
			var res = synAjaxJson("GET", '/v0.1/inport/select', 'goodsName='+name, "查询失败！");
			tab(res);
		}
    });
    /**
    ...
    table.on('tool(doEven)', function(obj){
        ...
        if(obj.event === 'del'){
          layer.confirm('是否删除', function(index){
        	  var r = synAjaxJson("DELETE", '/v0.1/inport/delete/'+data.id, null, "库存数量已卖出，数量不够，删除失败！");
        	  layer.close(index);
        	  if (r !=undefined) {
        		  obj.del();
        	  }
